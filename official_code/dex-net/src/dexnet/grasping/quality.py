    ...
    def grasp_quality(grasp, obj, params, contacts=None, vis=False):
        ...
        if contacts is None:
            ...
            if not contacts_found:
                ...
                # print(2222)
                return [np.zeros((3)),np.zeros((3))], np.zeros((3)), np.zeros((3)), np.zeros((3)), np.zeros((3)), 0

        if method == 'force_closure':
            # Use fast force closure test (Nguyen 1988) if possible.
            if len(contacts) == 2:
                c1, c2 = contacts
                return grasp.endpoints, c1.point, c2.point, c1.normal, c2.normal, PointGraspMetrics3D.force_closure(c1, c2, friction_coef)
